...
# 规则“带默认值的形参必须在没有默认值的形参之后”并不是一个笼统地应用于整个函数定义的规则，而是在每一个参数“区块”内独立生效的规则。
# 为了清晰地理解，首先要把函数签名看作被 / 和 * 分割成的三个独立区块：
# 位置限定参数块 (Positional-Only): 在 / 之前的所有参数。
# 标准参数块 (Positional-or-Keyword): 在 / 和 * 之间的所有参数。
# 关键字限定参数块 (Keyword-Only): 在 * 或 *args 之后的所有参数。
# 核心机制是：Python 解析器会从左到右检查【每一个区块】，并确保在【该区块内】，一旦出现带默认值的参数，其右侧的所有参数也必须有默认值。
# Python 强制要求所有“必填项”（无默认值的参数）必须出现在“选填项”（有默认值的参数）之前。
# 这样，当函数被调用时，传入的位置参数会从左到右依次填满所有必填的“坑位”，逻辑非常清晰。
