...
for dimension in dimensions:
    print(dimension)
# 和列表一样，可以用 for 循环遍历元组所有元素。
# 对于非修改性操作，元组与 list 一样，视为一个只读list。
# 元组不支持修改性操作，list 所有会修改 list 方法，元组都没有。
# 由于元组不可变，不需要预留空间给 append()，使得其占用内存较少，创建也略快。
# Python的字典（dict）和集合（set）要求它们的键（key）或元素必须是“可哈希的”（hashable）。
# 简单来说，一个对象在其生命周期内必须是不可变的，才能被哈希。
...
# Python 程序员会遵循一些格式设置约定。
# 学会编写整洁的 Python 代码后，就能明白他人编写的 Python 代码的整体结构——只要他们和你遵循相同的指南。
...
# 代码编写出来之后，调试时需要阅读；在给程序添加新功能时，也需要花很长的时间阅读；在与其他程序员共享代码时，他们也会阅读。

# PEP 8 建议每级缩进都使用 4 个空格。这既可提高可读性，又留下了足够的多级缩进空间。
# 在字处理文档中，常常使用制表符（tab）而不是空格来缩进。
...
# 专业程序员通常会在同一个屏幕上打开多个文件，使用标准行长可以让他们在屏幕上并排打开两三个文件时，同时看到各个文件的完整行。
# PEP 8 还建议注释的行长不超过 72 个字符，因为有些工具为大型项目自动生成文档时，会在每行注释开头添加格式化字符。
# PEP 8 中有关行长的指南并非不可逾越的红线，有些小组将最大行长设置为 99 个字符。
# 在学习期间，你不用过多考虑代码的行长，但别忘了，在协作编写程序时，大家几乎都遵守 PEP 8 指南。
# 在大多数编辑器中，可以设置一个视觉标志（通常是一条竖线），让你知道不能越过的界线在什么地方。

# 要将程序的不同部分分开，可使用空行。你绝对应该使用空行来组织程序文件，但是不要滥用。
# 只要按本书示例展示的那样做，你就能掌握其中的平衡。
