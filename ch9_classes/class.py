...
# 子类继承其父类的所有方法。但是，要使子类实例继承其父类的属性，必须在子类的初始化过程中调用__init__父类的方法，通常通过 super().__init__()。
# 这是处理继承的标准且正确的方法。调用父类的初始化方法，然后添加子类特有的属性。
# Python 中强烈推荐的最佳实践是在 __init__  方法内部初始化实例的所有属性。
# 即使属性的“真实”值稍后仅在另一种方法中确定，您仍应__init__使用默认值或占位符值（如None、0或空列表[]）来声明它。
...
# 这行代码让 Python 调用 Car 类的 __init__() 方法，从而让 ElectricCar 实例包含这个方法定义的所有属性。
...
# 可以使用标准库中的任何函数和类，只需在程序开头添加一条简单的import 语句即可。
...
# Python 的 random 主要模块使用了马特赛特旋转演算算法（Mersenne Twister）其核心生成器。
